...
import type { CreateEnumItemRequest } from "../model/types";

export const useCreateEnumItem = () => {
  const queryClient = useQueryClient();

  return useMutation<void, AxiosError, CreateEnumItemRequest>({
    mutationFn: (payload) => enumTypesService.createEnumItem(payload),
    onSuccess: () => {
      toast.success("Enum dəyəri uğurla yaradıldı");

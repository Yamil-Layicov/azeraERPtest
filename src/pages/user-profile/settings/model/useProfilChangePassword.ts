...
export const useProfileChangePassword = () => {
  ...
  const validateField = (fieldName: string, updatedFormData: FormData) => {
    ...
    if (!result.success) {
      result.error.errors.forEach((error) => {
        ...
        const displayField =
          field === "confirmNewPassword" ? "confirmPassword" : field;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    try {
      const response = await authService.profileChangePassword({
        currentPassword: formData.currentPassword,
        newPassword: formData.newPassword,
        confirmNewPassword: formData.confirmPassword,
      });
      ...
      if (!response.isSuccess) {
        toast.error(
          response.errorMessage || "Şifrə dəyişdirilərkən xəta baş verdi",
        );
        return;
      }
